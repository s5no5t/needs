var needs = {
  ...
  requirementsGraph: function(canvas){
    var that = {};
    ...
    that.update_graph_for_req = function(req){
      var nodes = {}, edges = {};
      var req_id = req.id.toString();

      nodes[req.id.toString()] = {label: req.id.toString() + " " + req.title.substring(0, 20), url: req.url};
      ...
      $(req.deriving_requirements).each(function(){
        ...
        edges[req_id][deriving_req_id] = {};
      });

      this.sys.merge({nodes: nodes, edges: edges});
    };

    that.update_graph_for_reqs = function(reqs){
      var nodes = {}, edges = {};
